...
func (a *AutoConfigManager) Run(testName string, autoConfigStrategyAgent strategies.Strategy, inputWorkload *workload.Workload) error {
	...
	for {
		...
		if reason, isValid := a.isConfigurationValid(newConfig); !isValid {
			fmt.Println("DONEEE2")
		...
		currentConfig = copyConfig(newConfig)
	}
...
func copyConfig(in map[string]*configuration.Configuration) map[string]*configuration.Configuration {
	out := make(map[string]*configuration.Configuration)
